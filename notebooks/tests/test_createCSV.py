...
def test_create_csv(tmp_path,dir_path,json_name):
    tmp = tmp_path / "sub"
    tmp.mkdir()
    json_path = os.path.join(os.getcwd(),"tests/json",json_name)
    dir_path = os.path.join(os.getcwd(),dir_path)
    createCSV(dir_path,tmp,json_path)
    val = pd.read_csv(os.path.join(tmp,"val.csv"))
...
def csv_generation(tmp_path_factory):
    def _csv_generation(dir_path,json_name):
        ...
        json_path = os.path.join(os.getcwd(),"tests/json",json_name)
        createCSV(dir_path=dir_path,outputPath=fn,json_path=json_path)
        return fn
    return _csv_generation

@pytest.mark.parametrize("dir_path,json_name", [("../test","jsonTest1.json"),("../test","jsonTest2.json")])
def test_no_empty_csv(csv_generation,dir_path,json_name):
...
@pytest.mark.parametrize("dir_path,json_name", [("../test","jsonTest1.json"),("../test","jsonTest2.json")])
def test_read_files(csv_generation,dir_path,json_name):
    path = csv_generation(dir_path,json_name)
    val = pd.read_csv(os.path.join(path,"val.csv"))
    train = pd.read_csv(os.path.join(path,"train.csv"))
...
@pytest.mark.parametrize("dir_path,json_name", [("../test","jsonTest1.json"),("../test","jsonTest2.json")])
def test_missed_files(csv_generation,dir_path,json_name):
    path = csv_generation(dir_path,json_name)
    val = pd.read_csv(os.path.join(path,"val.csv"))
    train = pd.read_csv(os.path.join(path,"train.csv"))
    df = pd.concat([train,val],ignore_index=True)
...
def test_create_csv_bad_json(csv_generation,dir_path,json_name):
    path = csv_generation(dir_path,json_name)
    val = pd.read_csv(os.path.join(path,"val.csv"))
    train = pd.read_csv(os.path.join(path,"train.csv"))
    df = pd.concat([train,val],ignore_index=True)
